...
embedding = KeyedVectors.load_word2vec_format(config.model_data_path + 'GoogleNews-vectors-negative300.bin', binary=True)
# embedding = torch.load(configs.model_data_path + 'GoogleNews-vectors-negative300.model').to(device)
...
def main():
    ...
    if last_epoch == epoch:
        torch.save(model, config.model_data_path + 'cpc_cnn.model')
    print("\ntimecost:", times, "\n")
